...
## SonarQube (optionnel sur le même job)

- Crée un credential **Secret text** d’ID `**sonar-token`** (token généré dans SonarQube).
- Lance le job avec le paramètre `**RUN_SONAR**` coché et `**SONAR_HOST_URL**` adapté. Voir [SONARQUBE.md](SONARQUBE.md).
